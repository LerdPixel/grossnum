...
class SGN : public GROSSNUM //указан класс-предок
{
private:
    int compare_modules(const SGN &x) const;
    SGN sub_modules(SGN &x);
    SGN add_modules(SGN &x);
    int sign;
public:
    SGN () {sign=0;}
    SGN (char *);
    SGN (int);
    void print(); //печать большого числа со знаком
    SGN friend operator - (SGN &x, SGN &y);
    SGN friend operator + (SGN &x, SGN &y);
    int operator > (const SGN &x) const;
    int operator < (const SGN &x) const;
    int operator <= (const SGN &x) const;
    int operator >= (const SGN &x) const;
    int operator == (const SGN &x) const;
...
SGN::SGN (int x) : GROSSNUM(x<0?-x:x)
{
    this->sign=x<0?1:0;
}
#endif
